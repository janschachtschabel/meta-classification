...
**Auto-optimization per run:** `C` (grid on validation), thresholds (global + per-label) and `min_samples_per_label` are determined automatically (the last is also settable per request). Selection and reported metrics always measure the decision rule serving actually applies (`decision_rule` in the metrics): tuned thresholds for multilabel, argmax for multiclass/binary (threshold tuning is skipped there — serving never reads thresholds for single-label tasks).
