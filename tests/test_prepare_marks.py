...
@pytest.mark.parametrize("synthetic_rows", ["train", "exclude"])
def test_a_holdout_scores_by_one_rule_with_and_without_the_generated_rows(tmp_path, synthetic_rows):
    """"exclude" drops every generated row, and here the last mark with them. The labels
    its test split has no row of still go unscored: "with" and "without" the AI rows are
    compared on one rule, not F1 0.0 against no F1 (review 2026-09-19 #1)."""
    rows = [[f"Bruchrechnung Aufgabe {i}", f"brüche{i}", "A", "", "", ""] for i in range(12)]
    ...
    prep = _prepare(tmp_path, _write(tmp_path, rows), cv_folds=0, stratified=True,
                    synthetic_rows=synthetic_rows, min_samples_per_label=1)
