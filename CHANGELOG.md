...
- **`excluded_generated_rows` counts a text once** under the dedupe, as the rows that
  would have trained; copies of a generated text were counted each.
- **The k-fold progress line says which rows validate** ("real rows validate" when the
  dataset has marks), as do the descriptions of the k-fold mode.
- **The real rows' label counts no longer copy the label matrix** (for the scored and
  the thin labels): one pass over the matrix itself.
...
the two. data-prep now enriches exact twins alike, so they stay one text and the rule
holds; a dataset enriched before that is best enriched again.
