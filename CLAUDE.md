...
## Architecture
- `app/routes/*` stay thin (auth, validation, HTTP mapping) and delegate to core modules (`data`, `dataset_stats`, `vectorizers`, `classifier`, `tuning`, `prepare`, `deploy`, `training`, `explain`, `predict_csv`, `evaluate`, `registry`, `model_io`, `model_archive`, `model_card`, `bundle_meta`, `model_report`, `jobs`, `job_history`, `feedback`, `sharing`, `errors`).
