...
- Dependencies: 12 runtime deps, pinned in `requirements.lock`; no new dependency without written justification.
...
- Model bundles = `config.json` + `metrics.json` + `head.skops` + `vectorizer.skops`. `model_io.py` owns the pickle-free (de)serialization + skops load-safety; `registry.py` owns the cache, disk locks and atomic publish (hidden `.name.tmp` dir + rename).
