...
## Architecture
- `app/routes/*` stay thin (auth, validation, HTTP mapping) and delegate to core modules (`data`, `dataset_stats`, `vectorizers`, `classifier`, `tuning`, `prepare`, `deploy`, `training`, `explain`, `registry`, `model_io`, `model_archive`, `model_card`, `jobs`, `sharing`, `errors`).
