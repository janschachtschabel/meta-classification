...
- Model selection splits the same way: `thresholds.py` owns where a decision cut sits (the grid search, the URI-keyed form a bundle stores, applying them) and is a leaf that imports nothing of ours; `tuning.py` owns which `C` wins and how the result is scored, and reads `thresholds` — never the reverse.
