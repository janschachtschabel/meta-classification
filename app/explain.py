...
def explain_prediction(model: ClassifierModel, text: str, top_n_words: int) -> dict:
    ...
    Returns the predictions, per-label confidence for ALL labels, and — for the
    top predicted labels — the words whose removal drops the confidence most
    (leave-one-out).
    """
    ...
    predicted = model.predict([text], include_baseline_diff=True)[0]
