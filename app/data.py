...
def count_rows(path: str | Path) -> int:
    """Data rows (excluding the header) of a CSV — a fast APPROXIMATION.

    Counts physical lines, so quoted multi-line fields (common in description
    columns) count as several rows; the exact parse count would require a full
    csv.reader pass. Display-only (dataset listings) — training parses properly.
