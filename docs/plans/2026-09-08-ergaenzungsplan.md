...
| ✅ B3 **measured 2026-09-10, adopted for auto/best** | **Iterative stratification** for the holdout split and the K folds (multilabel-aware; ~60 lines, no dependency) | Rare labels get positives in every fold → stabler per-label thresholds and metrics | ✅ Both: macro 0.7075 -> 0.7125 (+0.0050), spread across draws sd 0.0031 -> 0.0017, per-label F1 sd 0.0172 -> 0.0152. On for auto/best; `fast` (holdout) unmeasured — stratifying there moves rows between train/val/test and needs a fixed test split to compare fairly | S–M, 1–2 days |
