...
| `APIV3_TRAIN_MEMORY_MB` | `auto` | Memory budget for a training run, in MiB. It bounds the head-fit threads: every concurrent per-label fit holds ~2.5× the feature matrix in solver buffers, so a run that would outgrow the budget trains with fewer threads — slower, same model — instead of being OOM-killed. `auto` = 85 % of the container's cgroup memory limit (Docker `--memory`, a Kubernetes memory limit) when there is one, otherwise no cap; `0` = no cap. It also **refuses a model that cannot exist**: the fitted head is one float32 per label and feature and nothing releases it, so a run whose coefficients alone exceed the budget is stopped before its first fit with the numbers and the levers, rather than being OOM-killed minutes later. That is a label-count problem in practice — 8 279 labels over a 200 000-term vocabulary are 6.3 GB — and `min_samples_per_label` is the lever. `GET /config` shows the resolved `effective_train_memory_mb`. |
