...
| `APIV3_TRAIN_MEMORY_MB` | unset | Memory budget for a training run, in MiB. It bounds the head-fit threads: every concurrent per-label fit holds ~2.5× the feature matrix in solver buffers, so a run that would outgrow the budget trains with fewer threads — slower, same model — instead of being OOM-killed. Unset = 85 % of the container's cgroup memory limit (Docker `--memory`, a Kubernetes memory limit) when there is one, otherwise no cap; `0` = no cap. `GET /config` shows the resolved `effective_train_memory_mb`. |
